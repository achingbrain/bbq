...
bbq.ajax.BackButtonFixer = Class.create(/** @lends bbq.ajax.BackButtonFixer.prototype */ {
	_lastMethod: null,
	_neediFrameFix: null,
	_controller: null,
	...
	/**
	 * Provides support for the HTML5 History API to browsers that otherwise wouldn't support it.
	 *
	 * @param {Object} controller The object to call methods on
	 * @param {Function} emptyMethod The function to call to return the page to it's initial state
	 * @constructs
	 */
	...
	_callMethod: function(passed) {
		...
		if(passed) {
			...
			if(this._controller[method]) {
				...
				Log.warn("Method  " + method + " not found on controller");
			}
		} else if(this._emptyMethod) {
			//Log.debug("Called empty method");
